...
from pyrogram.types import CallbackQuery, InlineKeyboardButton, InlineKeyboardMarkup, ChatMemberUpdated
import asyncio
import config
...
async def welcome_bot(client: Client, chat_member_update: ChatMemberUpdated):
    # Check if the bot was added to a group
    if chat_member_update.new_chat_member.user.id == (await client.get_me()).id:
    ...
    if not await db.is_user_exist(chat_id):
        ...
        # Construct welcome message with buttons
        ...
        welcome_button = InlineKeyboardMarkup(
            [[
                InlineKeyboardButton("Support", url="https://twitter.com/messages/compose?recipient_id=1838594063273992192"),
        ...
        # await asyncio.sleep(30)  # Wait for 30 seconds (you can change this value)
        
        # # Delete the welcome message
        # await client.delete_messages(chat_id, welcome_text)
...
async def startprivate(client, message):
    ...
    welcomed = f"Hey <b>{message.from_user.first_name}</b>\nI'm a Based Telegram bot that will send latest events happening on BaseSpace. Made by @BaseSpace01.\n\n 🎚 use /settings for notification"
